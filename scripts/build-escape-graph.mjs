...
const [manifestPath='source-manifest.json', graphPath='source-graph.bin', outDir='generated/escape'] = process.argv.slice(2);
...
if(buf.length!==expected) throw new Error(\`Graph bytes mismatch \${buf.length} != \${expected}\`);
...
// Score direct positive LC4 -> DN input exactly from the real graph.
const direct=[];
for(const target of dnSet){
  let pos=0,neg=0,count=0;
  for(let e=rows[target];e<rows[target+1];e++){
    if(!loomSet.has(presyn[e])) continue;
    const w=weights[e]; count++;
    if(w>0) pos+=w; else neg+=Math.abs(w);
  }
  if(pos>0) direct.push({index:target,positiveWeight:pos,negativeWeight:neg,edgeCount:count});
}
...
for(const id of outputGroups) for(const i of groupMap.get(id)?.indices||[]) targetSet.add(i);

// Build compact source adjacency for reachability.
...
for(let target=0;target<n;target++){
  for(let e=rows[target];e<rows[target+1];e++) outTargets[cursor[presyn[e]]++]=target;
}
  ...
  for(let d=0;d<maxHops&&frontier.length;d++){
    ...
    for(const source of frontier){
      for(let e=outOffsets[source];e<outOffsets[source+1];e++){
        ...
        if(dist[t]===-1){dist[t]=d+1;next.push(t);}
      }
    }
    frontier=next;
  }
...
function bfsBackward(starts,maxHops){
  ...
  for(let d=0;d<maxHops&&frontier.length;d++){
    ...
    for(const target of frontier){
      for(let e=rows[target];e<rows[target+1];e++){
        const s=presyn[e];
        if(dist[s]===-1){dist[s]=d+1;next.push(s);}
      }
    }
    frontier=next;
  }
...
const remap=new Int32Array(n); remap.fill(-1);
...
let off=20;
for(const v of newRows){out.writeUInt32LE(v,off);off+=4;}
for(const v of edgeSources){out.writeUInt32LE(v,off);off+=4;}
for(const v of edgeWeights){out.writeDoubleLE(v,off);off+=8;}
...
for(const id of ['loom','turnL','turnR','flightL','flightR']){
  ...
  if(indices.length) keptGroups.push({id,label:src.label,indices,count:indices.length});
}
...
const readme=\`# FlyEye Escape Graph

...
function modeRules(){
  if(gameMode==='scare_fast'){
    return {
      title:lang==='zh'?'快速惊吓 · 排名规则':'SCARE FAST · RANKING',
      body:lang==='zh'
  ...
  return {
    title:lang==='zh'?'潜行靠近 · 排名规则':'SNEAK UP · RANKING',
    body:lang==='zh'
      ? '先看是否完成 20 秒且没有惊飞果蝇；同一结果下，谁靠得更近谁排名更高。再相同才比较坚持时间和最高威胁。积分只是表现分，不决定名次。'
      : 'First: finish the 20-second round without making the fly escape. Among equal outcomes, closer is better; then longer survival and lower peak threat break ties. Score is feedback only.',
    chips:lang==='zh'
      ? ['是否逃逸：未逃逸优先','最近距离：越小越好','时间：越长越好','最高威胁：越低越好','积分：接近70% + 生存25% + 稳定5%，逃逸−2500']
      : ['Escape: no escape ranks first','Closest: lower is better','Time: longer is better','Peak threat: lower is better','Score: 70% closeness + 25% survival + 5% control, −2500 if escaped'],
    leaderboard:lang==='zh'?'潜行靠近排行榜':'SNEAK UP LEADERBOARD',
    switchLabel:lang==='zh'?'玩「快速惊吓」':'PLAY SCARE FAST'
  };
}

function renderModeRules(){
  const rules=modeRules();
  const landingTitle=$('#landingRulesTitle'),landingBody=$('#landingRulesBody'),landingChips=$('#landingMetricChips');
  const resultTitle=$('#resultRulesTitle'),resultBody=$('#resultRulesBody'),resultChips=$('#resultMetricChips');
  const boardTitle=$('#leaderboardTitle'),switchBtn=$('#switchModeBtn');
  if(landingTitle)landingTitle.textContent=rules.title;
  if(landingBody)landingBody.textContent=rules.body;
  if(resultTitle)resultTitle.textContent=rules.title;
  if(resultBody)resultBody.textContent=rules.body;
  if(boardTitle)boardTitle.textContent=rules.leaderboard;
  if(switchBtn)switchBtn.textContent=rules.switchLabel;
  for(const host of [landingChips,resultChips]){
    if(!host)continue;
    host.innerHTML='';
    for(const chip of rules.chips){
      const span=document.createElement('span');span.textContent=chip;host.appendChild(span);
    }
  }
}
...
function updateResultUI(){
  ...
  $('#closestApproach')?.parentElement?.classList.toggle('hidden',gameMode==='scare_fast');
...
function buildLeaderboardRow(entry,{viewerCard=false}={}){
  ...
  if(gameMode==='scare_fast'){
    score.textContent=entry.escaped && entry.escape_latency_ms
      ? (entry.escape_latency_ms/1000).toFixed(2)+' s'
      : (lang==='zh'?'未触发':'NO ESCAPE');
  }else{
    score.textContent=entry.closest_approach==null?'—':Math.round(entry.closest_approach*100)+'%';
  }
  score.title=(lang==='zh'?'表现分 ':'Score ')+Number(entry.score).toLocaleString();
...
function flyVisionCopyKey(mode){
  return {
    human:'visionHumanExplain',
    stabilized:'visionStabilizedExplain',
    motion:'visionMotionExplain',
    loom:'visionLoomExplain',
    lc4:'visionLc4Explain'
  }[mode]||'visionHumanExplain';
}
function flyVisionLabelKey(mode){
  return {
    human:'visionHuman',
    stabilized:'visionStabilized',
    motion:'visionMotion',
    loom:'visionLoom',
    lc4:'visionLc4'
  }[mode]||'visionHuman';
}
function updateFlyVisionCopy(){
  const label=$('#visionModeLabel'),desc=$('#visionExplain');
  if(label)label.textContent=t(flyVisionLabelKey(flyVisionMode));
  if(desc)desc.textContent=t(flyVisionCopyKey(flyVisionMode));
}
function drawMosaic(){
  const layers=perceptionEngine.visual;
  const layer=layers?.[flyVisionMode]||lastFrame;
  if(!layer)return;

  const w=96,h=54;
  const data=new Uint8ClampedArray(w*h*4);
  for(let i=0;i<layer.length;i++){
    const v=layer[i]||0,p=i*4;
    if(flyVisionMode==='motion'){
      data[p]=Math.round(v*.18);data[p+1]=Math.round(v*.78);data[p+2]=v;
    }else if(flyVisionMode==='loom'){
      data[p]=Math.round(v*.90);data[p+1]=v;data[p+2]=Math.round(v*.18);
    }else if(flyVisionMode==='lc4'){
      data[p]=v;data[p+1]=Math.round(v*.34);data[p+2]=Math.round(v*.12);
    }else{
  ...
  visionLayerCtx.putImageData(new ImageData(data,w,h),0,0);
  mctx.clearRect(0,0,mosaicCanvas.width,mosaicCanvas.height);
  mctx.imageSmoothingEnabled=flyVisionMode==='human'||flyVisionMode==='stabilized';
  mctx.drawImage(visionLayerSource,0,0,mosaicCanvas.width,mosaicCanvas.height);

  const px=(rear?fly.x:1-fly.x)*mosaicCanvas.width;
  const py=fly.y*mosaicCanvas.height;
  mctx.save();
  mctx.strokeStyle='rgba(255,255,255,.72)';
  mctx.lineWidth=1.5;
  mctx.beginPath();mctx.arc(px,py,12,0,Math.PI*2);mctx.stroke();
  mctx.beginPath();mctx.moveTo(px-18,py);mctx.lineTo(px+18,py);mctx.moveTo(px,py-18);mctx.lineTo(px,py+18);mctx.stroke();
  mctx.restore();
  updateFlyVisionCopy();
}
...
async function submitFeedback(){
  ...
  if(!message){
    if(status) status.textContent=lang==='zh'?'请先填写反馈内容。':'Please enter feedback first.';
    $('#feedbackMessage')?.focus();
    return;
  }
  if($('#feedbackWebsite')?.value) return;
  if(button){button.disabled=true;button.textContent=t('feedbackSending')}
  ...
  try{
    const resp=await fetch('/api/feedback',{
      ...
      body:JSON.stringify({
        user_id:userId,
        display_name:playerName||undefined,
        category:$('#feedbackType')?.value||'other',
        message,
    ...
    if(status) status.textContent=t('feedbackSent');
    setTimeout(()=>$('#feedbackModal')?.classList.add('hidden'),850);
  }catch(err){
    console.warn('feedback submit failed',err);
    if(status) status.textContent=t('feedbackError');
  }finally{
    if(button){button.disabled=false;button.textContent=t('feedbackSubmit')}
  }
}
...
function switchToOtherGame(){
  gameMode=gameMode==='scare_fast'?'sneak_up':'scare_fast';
  applyModeUI();
  resetRound();
  if(stream && running){
    perceptionEngine.rearmAfterPractice(performance.now());
    ...
    $('#landing')?.classList.add('hidden');
    $('#hud')?.classList.remove('hidden');
    toast(gameMode==='scare_fast'?t('modeScareTitle'):t('modeSneakTitle'));
  }else{
    $('#landing')?.classList.remove('hidden');
  }
}
...
$('#gameHomeBtn').onclick=returnToGameSelection;
$('#gameSwitchBtn').onclick=switchToOtherGame;
$('#allGamesBtn').onclick=returnToGameSelection;
$('#switchModeBtn').onclick=switchToOtherGame;
...
document.querySelectorAll('[data-vision]').forEach(btn=>{
  btn.onclick=()=>{
    flyVisionMode=btn.dataset.vision||'human';
    document.querySelectorAll('[data-vision]').forEach(b=>b.classList.toggle('active',b===btn));
    drawMosaic();
  };
});
...
$('#feedbackBtn').onclick=()=>{$('#feedbackModal').classList.remove('hidden');setTimeout(()=>$('#feedbackMessage')?.focus(),0)};
$('#closeFeedback').onclick=()=>$('#feedbackModal').classList.add('hidden');
$('#feedbackModal').onclick=(event)=>{if(event.target===$('#feedbackModal')) $('#feedbackModal').classList.add('hidden')};
